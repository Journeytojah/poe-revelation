...
function removeHeadersAtOffsets(offsets: number[], headers: Header[]) {
  for (let idx = 0; idx < headers.length;) {
    ...
    if (
      header.name != null &&
      offsets.some((offset) => offset >= header.offset && offset < header.offset + header.length)
    ) {
...
export function createHeaderFromSelected(selection: boolean[], headers: Header[]) {
  ...
  const emptyHeader = headers.find(
    (header) =>
      selected[0] >= header.offset && selected[0] + selected.length <= header.offset + header.length
  )!;
